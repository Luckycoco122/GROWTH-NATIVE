...
/* ========= MÓDULO: Header / Navbar / Overlay / Sticky ========= */
function initHeaderNavbar() {
  ...
  const onScroll = () => {
    const active = (window.scrollY || document.documentElement.scrollTop) >= 400;
    header?.classList.toggle('active', active);
    goTopBtn?.classList.toggle('active', active);
    repaintBrand();
  };
  on(window, 'scroll', onScroll, { passive: true });
  on(window, 'load', onScroll, { once:true });
...
function initCarousel(){
  ...
  roots.forEach(root => {
    ...
    function measurePeriod(){
      ...
      if (!period){
        period = originals.reduce((acc, sl, i) =>
          acc + sl.getBoundingClientRect().width + (i<originals.length-1?GAP:0), 0);
      }
    }
    ...
    let rt; on(window, 'resize', () => {
      clearTimeout(rt); rt = setTimeout(()=>{ stop(); track.scrollLeft=0; measurePeriod(); start(); },150);
    }, { passive:true });
    ...
    const scrollToHeading = () => {
      ...
      if (!t) return;
      const headerH = ($('[data-header]')?.offsetHeight) || 0;
...
/* ========= Loader de parciales (header/footer) ========= */
...
/* ========= Bootstrap: espera DOM + parciales y luego inicia ========= */
async function initApp() {
  // header/footer ya están en el DOM en este punto
  initHeaderNavbar();
  initContactForm();
  initCarousel();
  initCounters();
